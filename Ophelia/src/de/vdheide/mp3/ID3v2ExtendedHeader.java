...
public class ID3v2ExtendedHeader {
     ...
     *            Use CRC?
     * @param crc
     *            CRC of frames (will be set to 0 if <code>use_crc</code>==
     *            false)
     * @param padding_size
     *            Size of padding
     */
    public ID3v2ExtendedHeader(boolean use_crc, int crc,
            int padding_size) {
        crc_present = use_crc;
        if (crc_present == true) {
            this.crc = crc;
    ...
    public ID3v2ExtendedHeader(InputStream in) throws IOException {
        ...
        padding_size = (int) (new de.vdheide.utils.Bytes(head, 6, 4)
                .getValue());
        ...
        if (crc_present == true) {
            ...
            in.read(crc_array);
            crc = (int) new de.vdheide.utils.Bytes(crc_array)
                    .getValue();
        }
    ...
    public byte[] getBytes() {
        ...
        if (crc_present == true) {
            byte[] crc_byte = (new de.vdheide.utils.Bytes(crc, 4))
                    .getBytes();
